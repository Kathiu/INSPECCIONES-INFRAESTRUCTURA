(() => {
  ...
  button.onclick = async () => {
    ...
    try {
      ...
      let html=`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Reporte mensual ${esc(mes)} ${anio}</title><style>body{font-family:Arial,sans-serif;color:#17202a;margin:0}header{text-align:center;padding:30px;border-bottom:3px solid #0b57d0}h1{font-size:24px}h2{background:#e8eef8;padding:10px;margin-top:24px;margin-bottom:10px;break-after:avoid-page;page-break-after:avoid;orphans:3;widows:3}h3{margin:0 0 8px}.page{padding:0 28px}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:22px 0}.box{border:1px solid #ccc;padding:12px;text-align:center}.num{font-size:22px;font-weight:bold}.item{border:1px solid #ccd2d8;padding:14px;margin:12px 0;break-inside:avoid;page-break-inside:avoid}.photos{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:10px}.photos img{width:100%;height:250px;object-fit:contain;border:1px solid #ddd;display:block}.small{font-size:12px;color:#555}@media print{body{margin:0}.page{padding:0 12mm}h2{break-after:avoid-page;page-break-after:avoid}.item{break-inside:avoid;page-break-inside:avoid}}@media(max-width:700px){.summary{grid-template-columns:repeat(2,1fr)}.photos{grid-template-columns:1fr}}</style></head><body><header><h1>REPORTE MENSUAL DE INSPECCIÓN DE INFRAESTRUCTURA</h1><b>MES: ${esc(mes)} &nbsp; AÑO: ${anio}</b><div class="small">Generado: ${new Date().toLocaleString('es-PE')}</div></header><div class="page"><div class="summary"><div class="box"><div class="num">${rows.length}</div>Inspecciones</div><div class="box"><div class="num">${zones.length}</div>Áreas</div><div class="box"><div class="num">${points.length}</div>Puntos</div><div class="box"><div class="num">${incidence}</div>Incidencias</div></div><div class="summary"><div class="box"><div class="num">${pending}</div>Pendientes</div><div class="box"><div class="num">${rows.filter(x=>x.resultado==='SIN INCIDENCIA').length}</div>Sin incidencia</div><div class="box"><div class="num">${photos}</div>Fotografías</div><div class="box"><div class="num">${rows.filter(x=>x.sync==='SINCRONIZADO').length}</div>En Drive</div></div>`;
      let last='';
      rows.forEach(x=>{if(x.zona!==last){html+=`<h2>ÁREA / ZONA: ${esc(x.zona)}</h2>`;last=x.zona}html+=`<div class="item"><h3>PUNTO DE INSPECCIÓN: ${esc(x.punto)}</h3><b>Fecha:</b> ${new Date(x.fecha).toLocaleString('es-PE')}<br><b>Resultado:</b> ${esc(x.resultado)}<br>${x.estado?`<b>Estado:</b> ${esc(x.estado)}<br>`:''}<b>Observación:</b> ${esc(x.observacion||'Sin incidencia')}<br><b>Fotografías:</b> ${(x.fotos||[]).length}<div class="photos">${(x.fotos||[]).map(f=>`<img src="${f}">`).join('')}</div></div>`});
      html+='</div></body></html>';
