...
function doPost(e) {
  try {
    ...
    if (data.action !== 'syncInspection') throw new Error('Acción no válida.');
    ...
    const month = String(data.mes || 'SIN_MES').replace(/[\\/:*?"<>|]/g, '_');
    ...
    // Idempotencia: si ya existe el JSON de esta inspección, no se vuelve a duplicar.
    const jsonName = safe_(data.id) + '.json';
    const existing = inspectionFolder.getFilesByName(jsonName);
    if (existing.hasNext()) {
      return json_({ok:true,alreadySynced:true,id:data.id});
    }
