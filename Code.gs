...
function syncReport_(data) {
  ...
  // Elimina versiones anteriores del mismo reporte para evitar duplicados.
  ['.pdf', '.html'].forEach(ext => {
    const old = monthFolder.getFilesByName(base + ext);
    while (old.hasNext()) old.next().setTrashed(true);
  });

  // Guarda el HTML como respaldo técnico.
  monthFolder.createFile(base + '.html', html, MimeType.HTML);

  // Convierte el HTML completo a PDF y guarda el PDF real en Drive.
  const pdfBlob = Utilities.newBlob(html, 'text/html', base + '.html')
    .getAs(MimeType.PDF)
    .setName(base + '.pdf');
