const CACHE='inspecciones-v15';
const FILES=['./','./index.html','./styles.css','./app.js','./report-drive.js','./catalog.json','./manifest.json'];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(FILES)).then(()=>self.skipWaiting())));
self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key!=='inspecciones-v15').map(key=>caches.delete(key)))).then(()=>self.clients.claim())));
